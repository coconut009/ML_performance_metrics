...
def ada_boost(x_train,y_train,x_test,y_test):    
    ...
    trainT  =   t1-t0
    runT    =   t2-t0
    return tn, fp, fn, tp , trainT, runT
...
for i in range (2,7):
    ...
    for train_index, test_index in kf.split(attributes_1):
        ...
        t2 = time()
        print ("At %d number of fold on data set 1, the confusion matrix of Adaboost algorithm: tn: %d, fp:%d fn:%d, tp:%d  " %(counter,ada_boost_1[0],ada_boost_1[1],ada_boost_1[2],ada_boost_1[3]))
        print ("At %d number of fold on data set 1, the confusion matrix of SVM algorithm: tn: %d, fp:%d fn:%d, tp:%d " %(counter,svm_1[0],svm_1[1],svm_1[2],svm_1[3]))
